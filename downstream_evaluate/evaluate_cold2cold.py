...
def eva_cold2cold(data_path, node_path, label_path):
    # 读取原始矩阵 (20+18)*2
    ...
    for i in range(len(df_data_extract)):
        ...
        if "冷通道" in name:  # 仅考虑上游对下游是否判错
            cold2cold_label = label.loc[name]   # 提取该冷通道的 label
            
            selected_label = cold2cold_label[cold2cold_label==1].index.tolist()  # 选出该冷通道《应该》影响的冷通道清单。
...
if __name__ == "__main__":
    ...
    # for i in [6]:
        ...
        wrong_num, right_num = eva_cold2cold(data_path, node_path, label_path)
